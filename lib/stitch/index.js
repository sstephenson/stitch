(function() {
  var CoffeeScript, Package, compileCache, compileFile, compilerIsAvailableFor, defaultCompilers, expandPaths, exports, extend, extname, forEachAsync, fs, gatherSource, gatherSources, gatherSourcesFromPath, getCompiledSourceFromCache, getCompilersFrom, getFilesInTree, getRelativePath, join, merge, mtimeCache, normalize, putCompiledSourceToCache, stitch, stripExtension, sys, walkTree, _ref;
  ...
  module.exports = exports = stitch = function(options, callback) {
    var _ref, _ref2;
    (_ref = options.identifier) != null ? _ref : options.identifier = 'require';
    (_ref2 = options.paths) != null ? _ref2 : options.paths = ['lib'];
  ...
  exports.getRelativePath = getRelativePath = function(paths, path, callback) {
    path = normalize(path);
    return expandPaths(paths, function(err, expandedPaths) {
      if (err) {
        return callback(err);
      }
  ...
  gatherSource = function(path, options, callback) {
    return getRelativePath(options.paths, path, function(err, relativePath) {
      if (err) {
        return callback(err);
      } else {
  ...
  exports.gatherSources = gatherSources = function(options, callback) {
    ...
    paths = options.paths;
    sources = {};
    return forEachAsync(paths, function(next, sourcePath) {
      if (next) {
