(function() {
  var Package, async, compileCache, extname, fs, join, mtimeCache, normalize, stitch, _, _ref;
  ...
  module.exports = Package = (function() {
    function Package(config) {
      ...
      this.extensions = _.extend({}, require.extensions, stitch.extensions, config.extensions);
    ...
    Package.prototype.gatherCompilableSource = function(sources, path, callback) {
      if (this.extensions[extname(path)]) {
    ...
    Package.prototype.compileFile = function(path, callback) {
      ...
      if (this.cache && compileCache[path] && mtimeCache[path] === compileCache[path].mtime) {
        return callback(null, compileCache[path].source);
      } else if (compile = this.extensions[extension]) {
        source = null;
        mod = {
        ...
        try {
          compile(mod, path);
          if (this.cache && (mtime = mtimeCache[path])) {
            compileCache[path] = {
              mtime: mtime,
              source: source
            };
