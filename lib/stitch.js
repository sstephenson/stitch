(function() {
  var CoffeeScript, _ref, compileCache, compileFile, compilerIsAvailableFor, defaultCompilers, expandPaths, extend, extname, forEachAsync, fs, gatherSource, gatherSources, gatherSourcesFromPath, getCompiledSourceFromCache, getCompilersFrom, getFilesInTree, getRelativePath, join, merge, normalize, putCompiledSourceToCache, stitch, stripExtension, sys, walkTree;
  ...
  compileCache = {};
  getCompiledSourceFromCache = function(path, stat) {
    var cache;
    return (cache = compileCache[path]) ? (stat.mtime.toString() === cache.mtime.toString() ? cache.source : null) : null;
  };
  putCompiledSourceToCache = function(path, stat, source) {
  ...
  stitch.compileFile = (compileFile = function(path, options, callback) {
    return fs.stat(path, function(err, stat) {
      ...
      if (err) {
        ...
        if (options.cache && (source = getCompiledSourceFromCache(path, stat))) {
