(function() {
  var CoffeeScript, compileCache, compileFile, compilerIsAvailableFor, defaultCompilers, expandPaths, extend, extname, forEachAsync, fs, gatherPackageSources, gatherSource, gatherSourcePaths, gatherSources, gatherSourcesFromNpmPackage, gatherSourcesFromPath, getCompiledSourceFromCache, getCompilersFrom, getFilesInTree, getRelativePath, join, merge, mtimeCache, normalize, npm, putCompiledSourceToCache, stitch, stripExtension, sys, walkTree, _ref;
  ...
  forEachAsync = function(elements, callback) {
    var element, key, next, remainingCount, value, _i, _len, _results, _results2;
    remainingCount = 0;
    next = function() {
      remainingCount--;
      if (remainingCount <= 0) {
    ...
    if (elements.length != null) {
      ...
      for (key in elements) {
        value = elements[key];
        _results2.push(callback(next, key, value));
      }
      return _results2;
    }
  };
  module.exports = stitch = function(options, callback) {
    ...
    (_ref4 = options.packages) != null ? _ref4 : options.packages = {};
  ...
  stitch.gatherSources = gatherSources = function(options, callback) {
    var sources;
    sources = {};
    return gatherPackageSources(sources, options, function(err, sources) {
      if (err) {
        return callback(err);
      } else {
        return gatherSourcePaths(sources, options, function(err, sources) {
          if (err) {
            return callback(err);
          } else {
            return callback(null, sources);
  ...
  gatherSourcePaths = function(sources, options, callback) {
    var sourcePaths;
    sourcePaths = options.sourcePaths;
    sourcePaths != null ? sourcePaths : sourcePaths = [];
  ...
  gatherPackageSources = function(sources, options, callback) {
    var packages;
    ...
    if (Object.keys(packages).length === 0) {
      ...
      return forEachAsync(packages, function(next, pkg, version) {
        if (next) {
          return gatherSourcesFromNpmPackage(pkg, version, function(err, pathSources) {
            var key, value;
            if (err) {
              return callback(err);
            } else {
  ...
  gatherSourcesFromNpmPackage = function(pkg, version, callback) {
    var jsonFile, packagePath;
    ...
    return fs.readFile(jsonFile, 'utf8', function(err, data) {
      var mainFilename, package;
      if (err) {
        return callback(err);
      }
      package = JSON.parse(data);
      mainFilename = join(packagePath, package.main);
      return getRelativePath([packagePath], mainFilename, function(err, relativePath) {
        var mainRequirePath, pathSources;
        if (err) {
          return callback(err);
        }
        ...
        return fs.readFile(mainFilename, 'utf8', function(err, source) {
          if (err) {
            return callback(err);
          }
          ...
          return callback(null, pathSources);
        });
      });
    });
  };
