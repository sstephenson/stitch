(function() {
  var CoffeeScript, Package, async, compilers, eco, extname, fs, join, jsmin, normalize, _, _ref;
  ...
  exports.Package = Package = (function() {
    function Package(config) {
      ...
      this.compress = (config.compress && jsmin) || false;
      this.cache = (_ref5 = config.cache) != null ? _ref5 : true;
      this.mtimeCache = {};
      this.compileCache = {};
    }
    Package.prototype.compile = function(callback) {
      return async.parallel([this.compileDependencies, this.compileSources], __bind(function(err, parts) {
        var sources;
        if (err) {
          return callback(err);
        } else {
