(function() {
  var CoffeeScript, _ref, compileCache, compileFile, compilerIsAvailableFor, defaultCompilers, expandPaths, extend, extname, forEachAsync, fs, gatherSource, gatherSources, gatherSourcesFromPath, getCompiledSourceFromCache, getCompilersFrom, getFilesInTree, getRelativePath, join, merge, mtimeCache, normalize, putCompiledSourceToCache, stitch, stripExtension, sys, walkTree;
      ...
      return forEachAsync(files, function(next, file) {
        ...
        if (next) {
          ...
          return fs.stat(filename, function(err, stats) {
            mtimeCache[filename] = (typeof stats === "undefined" || stats === null) ? undefined : stats.mtime == null ? undefined : stats.mtime.toString();
  ...
  getCompiledSourceFromCache = function(path) {
    var cache;
    return (cache = compileCache[path]) ? (mtimeCache[path] === cache.mtime ? cache.source : null) : null;
  };
  putCompiledSourceToCache = function(path, source) {
    var mtime;
    return (mtime = mtimeCache[path]) ? (compileCache[path] = {
      mtime: mtime,
      source: source
    }) : null;
  };
