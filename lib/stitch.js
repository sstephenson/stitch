(function() {
  var CoffeeScript, _a, compileFile, compilerIsAvailableFor, defaultCompilers, expandPaths, extend, extname, forEachAsync, fs, gatherSource, gatherSources, gatherSourcesFromPath, getCompilersFrom, getFilesInTree, getRelativePath, join, merge, normalize, stripExtension, sys, walkTree;
  ...
  defaultCompilers = {
    js: function(source, callback) {
      return callback(false, source);
    }
  };
  try {
    CoffeeScript = require('coffee-script');
    defaultCompilers.coffee = function(source, callback) {
      try {
        return callback(false, CoffeeScript.compile(source));
      } catch (err) {
        return callback(err);
      }
    };
  ...
  exports.compileFile = (compileFile = function(path, options, callback) {
    ...
    return fs.readFile(path, function(err, contents) {
      ...
      if (err) {
        ...
        return (compile = compilers[extension]) ? compile(source, callback) : callback("no compiler for '." + (extension) + "' files");
      }
    });
  });
